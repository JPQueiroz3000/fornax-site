...
document.getElementById('cep-destino').addEventListener('input', function(e) {
...
async function calculateShipping(formData) {
    ...
    const productData = {
        ...
        length: parseInt(selectedOption.dataset.length),
        weight: parseFloat(selectedOption.dataset.weight),
        price: parseFloat(selectedOption.dataset.price)
    };
    
    const quantity = parseInt(formData.get('quantidade'));
    const cepOrigem = formData.get('cep-origem').replace(/\D/g, '');
    const cepDestino = formData.get('cep-destino').replace(/\D/g, '');
...
function simulateAPIResponse(requestData) {
    return new Promise((resolve) => {
        setTimeout(() => {
            const basePrice = requestData.products[0].insurance_value * 0.1; // 10% do valor do produto
...
document.getElementById('shippingForm').addEventListener('submit', async function(e) {
    ...
    // Validações
    const cepDestino = formData.get('cep-destino');
    ...
    try {
        const shippingOptions = await calculateShipping(formData);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        calculatorContainer.style.transition = 'opacity 0.6s ease, transform 0.6s ease';
